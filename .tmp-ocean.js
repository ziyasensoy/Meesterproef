...
export class OceanAnimation {
  ...
  createParticle(randomY = false) {
    ...
    return {
      ...
      y: randomY ? Math.random() * (h + 200) - 100 : -length - Math.random() * 100,
      length,
      speed,
      opacity,
      angle,
  ...
  frame() {
    ...
    this.driftX = Math.sin(this.time * 0.0003) * 8 + Math.sin(this.time * 0.0007) * 4 + velDrift * 0.3;
  ...
  drawCaustics(width, height, depth) {
    const { ctx } = this;
    ...
    for (let i = 0; i < 5; i++) {
      const cx = width * (0.2 + i * 0.15) + Math.sin(this.time * 0.001 + i) * 40;
      const cy = height * 0.3 + Math.cos(this.time * 0.0008 + i * 2) * 30;
